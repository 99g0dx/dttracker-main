...
export function SoundIngest({ onSuccess, onSoundDetected }: Props) {
  const [url, setUrl] = useState('')
  const [loading, setLoading] = useState(false)
  const [status, setStatus] = useState<'idle' | 'analyzing' | 'indexing' | 'detected'>('idle')
  const [resolvedSound, setResolvedSound] = useState<any>(null)
  ...
  const handleIngest = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('sound-tracking', {
        body: { action: 'ingest', url: url.trim() },
      })
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-border bg-input px-3 py-3 pl-11 text-sm text-foreground placeholder:text-slate-500 focus:border-primary focus-visible:ring-2 focus-visible:ring-primary/40 outline-none transition"
              />
            </div>
