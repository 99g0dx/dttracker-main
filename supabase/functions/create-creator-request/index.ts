...
serve(async (req) => {
  ...
  try {
    const { request_id, agencyEmail } = await req.json();
...
<html>
  <body style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #f4f4f7; padding: 40px 20px; color: #333;">
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'onboarding@resend.dev', // Update with your verified sender email
        to: [agencyEmail || 'idrissolomonsu@yahoo.com'], // Fallback to default email
