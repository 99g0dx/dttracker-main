...
serve(async (req) => {
  ...
  try {
    ...
    if (!activation_id) {
      return new Response(
        JSON.stringify({ error: 'activation_id required' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    const { data: activation, error: actError } = await supabase
      .from('activations')
      .select('id, type, auto_approve, payment_per_action, base_rate, max_posts_per_creator')
      .eq('id', activation_id)
      .single();
    ...
    if (activation.type === 'contest') {
      ...
      if (creator_id) {
        ...
        return new Response(
          JSON.stringify({
            error: 'creator_id or (creator_handle + creator_platform) required for contest submissions',
          }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
