...
import type { Sound, SoundVideo, SoundWithVideos, ApiResponse, ApiListResponse } from '../types/database'
...
export async function list(): Promise<ApiListResponse<SoundWithVideos>> {
  try {
    ...
    const soundsWithStats: SoundWithVideos[] = (sounds || []).map((sound: any) => {
      const videos = sound.videos || []
      return {
        ...sound,
        videos,
        videos_count: videos.length,
        total_views: videos.reduce((sum: number, v: SoundVideo) => sum + v.views, 0),
 ...
 */
export async function getById(soundId: string): Promise<ApiResponse<SoundWithVideos>> {
  try {
 ...
 */
export async function unlinkFromCampaign(campaignId: string): Promise<ApiResponse<void>> {
  try {
    const { error } = await supabase
      .from('campaigns')
      .update({
 ...
 */
export async function refreshSound(soundId: string): Promise<ApiResponse<Sound>> {
  try {
