...
export function Payment({ onNavigate }: PaymentProps) {
  ...
  const tierFromParam =
    tierOptions.find((tier) => planParts.includes(tier)) || "pro";
  const cycleParam = searchParams.get("cycle")?.toLowerCase() as
    | BillingCycle
    | undefined;
  const cycleFromParam = cycleOptions.find((cycle) =>
    planParts.includes(cycle),
  );
  const initialBillingCycle =
    (cycleParam && cycleOptions.includes(cycleParam)
      ? cycleParam
      : cycleFromParam) || "monthly";
  ...
  const MAX_EXTRA_SEATS = 2;
  const initialExtraSeats = Math.min(
    MAX_EXTRA_SEATS,
    Math.max(0, Number(searchParams.get("seats") || 0)),
  );
  ...
  const allowExtraSeats =
    isPaidTier && selectedPlan?.extra_seat_price_cents !== null;
  ...
  const initialProviderParam = (
    searchParams.get("provider") || "paystack"
  ).toLowerCase();
  ...
  const planHighlights = useMemo(() => {
    ...
    const highlights = [
      ...
      formatLimit(
        selectedPlan.max_creators_per_campaign,
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between items-start">
                  ...
                  <p className="font-semibold text-foreground">
                    {selectedPlan
                      ? formatPrice(selectedPlan.base_price_cents, currency)
                      : "--"}
                  </p>
                </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="bg-card border-border">
            <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {allowExtraSeats
                          ? `Extra seats (max 2) are ${formatPrice(extraSeatPrice, currency)} each.`
                    ...
                    {allowExtraSeats ? (
                      <div className="flex items-center gap-3">
                        <Button
                          ...
                          onClick={() =>
                            setExtraSeats(Math.max(0, extraSeats - 1))
                          }
                        >
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setExtraSeats(
                              Math.min(MAX_EXTRA_SEATS, extraSeats + 1),
                            )
                          }
                        >
              ...
              <p className="text-xs text-center text-muted-foreground mt-4">
                ...
                Policy. Your subscription will automatically renew each billing
                cycle until canceled.
              </p>
            </CardContent>
          </Card>
          ...
          <div className="flex items-start gap-3 p-4 bg-muted/40 rounded-lg border border-border/60">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {provider === "stripe" ? "Stripe" : "Paystack"}. We never see or
          ...
          <div className="flex items-center justify-center gap-4 pt-4 flex-wrap">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {provider === "stripe" ? (
                <>
                  <div className="px-2 py-1 bg-muted/60 rounded text-xs text-muted-foreground">
                    Visa
                  </div>
                  <div className="px-2 py-1 bg-muted/60 rounded text-xs text-muted-foreground">
                    Mastercard
