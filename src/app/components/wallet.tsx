    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 sm:gap-4">
        <button
          ...
          className="w-11 h-11 min-h-[44px] rounded-md bg-white/[0.03] hover:bg-white/[0.06] active:bg-white/[0.08] border border-white/[0.08] flex items-center justify-center transition-colors"
          aria-label="Back"
        >
      ...
      <div className="grid gap-4 sm:gap-5 lg:gap-6 grid-cols-1 sm:grid-cols-2">
        <Card className="bg-[#0D0D0D] border-white/[0.08] rounded-xl overflow-hidden" style={{ boxShadow: 'var(--shadow-card)' }}>
          <CardContent className="p-5 sm:p-6 lg:p-7">
            ...
            {balanceLoading ? (
              <div className="h-12 sm:h-14 bg-white/[0.04] rounded animate-pulse" />
            ) : (
          ...
          <CardContent className="p-5 sm:p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500">Expected Locked (from activations)</span>
                <span className="text-sm font-medium text-white">
                  {formatAmount(syncState.expected_locked_from_activations, currency)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500">Actual Locked</span>
                <span className="text-sm font-medium text-white">
              ...
              {Math.abs(syncState.locked_discrepancy) >= 0.01 && (
                <>
                  <div className="flex justify-between items-center pt-2 border-t border-white/[0.08]">
                    <span className="text-xs text-amber-400">Discrepancy</span>
                  ...
                  {Math.abs(syncState.locked_discrepancy) <= autoFixThreshold && (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="w-full border-white/[0.08] mt-2"
                    >
              ...
              {syncState.live_activation_count > 0 && (
                <div className="flex justify-between items-center pt-2 border-t border-white/[0.08]">
                  <span className="text-xs text-slate-500">Live Activations</span>
                  <span className="text-sm font-medium text-white">
      ...
      <Card className="bg-[#0D0D0D] border-white/[0.08] rounded-xl overflow-hidden" style={{ boxShadow: 'var(--shadow-card)' }}>
        <CardContent className="p-5 sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-4 sm:mb-5">
            <h2 className="text-lg sm:text-xl font-semibold text-white">Transactions</h2>
            <div className="flex items-center gap-2">
              <DropdownMenu open={filterOpen} onOpenChange={setFilterOpen}>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    size="sm"
                    className={`border-white/[0.08] ${hasFilters ? "bg-primary/10 border-primary/30 text-primary" : ""}`}
                  >
                    <Filter className="w-4 h-4 mr-2" />
                    Filter
                    {hasFilters && (
                      <span className="ml-2 px-1.5 py-0.5 text-xs rounded-full bg-primary text-black font-medium">
                        {filterType.length + (dateStart || dateEnd ? 1 : 0) + (minAmount !== "" || maxAmount !== "" ? 1 : 0)}
                      </span>
                    )}
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-72 p-0 bg-[#0D0D0D] border-white/[0.08]">
                  {/* Header */}
                  ...
                  <div className="px-4 py-3 border-b border-white/[0.06]">
                    <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Transaction Type</span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <Input
                          ...
                          className="bg-white/[0.04] border-white/[0.08] text-white text-xs h-8 w-full"
                        />
                      </div>
                      <div>
                  ...
                  <div className="px-4 py-3 border-b border-white/[0.06]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Amount ({currency})</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="relative">
                        ...
                        <Input
                          ...
                          className="bg-white/[0.04] border-white/[0.08] text-white text-xs h-8 pl-6"
                        />
                      </div>
                      <div className="relative">
                        <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-slate-500">₦</span>
                  ...
                  <div className="px-4 py-3 flex gap-2">
                    <Button
                      ...
                      className="flex-1 h-8 text-xs border-white/[0.08]"
                    >
                      Cancel
                    </Button>
                    <Button
          ...
          {hasFilters && (
            <div className="flex flex-wrap items-center gap-2 mt-4 mb-2">
              ...
              {filterType.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-white/[0.06] text-slate-300"
                >
                  {getTransactionLabel(t)}
                  <button
              ...
              {(dateStart || dateEnd) && (
                <span className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-white/[0.06] text-slate-300">
                  {dateStart && dateEnd
                    ? `${dateStart} - ${dateEnd}`
                    : dateStart
                    ? `From ${dateStart}`
      ...
      <Dialog open={autoFixDialogOpen} onOpenChange={setAutoFixDialogOpen}>
        <DialogContent className="bg-[#0D0D0D] border-white/[0.08]">
          <DialogHeader>
            <DialogTitle className="text-white">Auto-fix Sync Discrepancy</DialogTitle>
            <DialogDescription className="text-slate-400">
            ...
            {syncState && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">Current Discrepancy</span>
                  <span className="text-white font-medium">
                    {formatAmount(Math.abs(syncState.locked_discrepancy), currency)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">Auto-fix Threshold</span>
                  <span className="text-white font-medium">
