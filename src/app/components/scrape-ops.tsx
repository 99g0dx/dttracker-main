...
export function ScrapeOps({ onNavigate }: ScrapeOpsProps) {
  ...
  const { data: runs = [], isLoading: runsLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("scrape_runs")
        .select("id, job_id, actor_id, status, started_at, finished_at, duration_ms, items_count, error_raw, error_type, is_valid, scrape_jobs(platform)")
  ...
  const summary = useMemo(() => {
    ...
    const valid = inRange.filter((r) => r.is_valid === true).length;
    ...
    return { total, succeeded, valid, byPlatform, byActor };
  }, [runs]);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card className="bg-slate-900/50 border-slate-700">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              <p className="text-slate-400 text-sm">Valid data</p>
              <p className="text-xl font-medium text-blue-400">{summary.valid}</p>
            </div>
            <div>
                  ...
                  <tbody>
                    {(runs as RunRow[]).map((r) => (
                      <tr key={r.id} className="border-b border-slate-800">
                        ...
                        <td className="py-2 pr-2">
                          <span className={r.is_valid ? "text-emerald-400" : "text-slate-500"}>
                            {r.is_valid ? "Yes" : "No"}
                          </span>
                        </td>
                        <td className="py-2 pr-2 text-slate-400">{r.error_type ?? "—"}</td>
